...
//         System.out.println(e);
   ...
   private void parsWohnung(String[] d) {
      short wNummer = Short.parseShort(d[1]);
      short hausNr = Short.parseShort(d[4]);
      Wohnung wohnung = ds.getWohnung(wNummer);
      wohnung.bezeichnung = d[2];
      wohnung.beschreibung = d[3];
      wohnung.hausnummer = hausNr;
      wohnung.faeche = Double.parseDouble(d[5]);
      wohnung.bewertung = Double.parseDouble(d[6]);
   }

   private void parsInvestition(String[] d) {
      Investition iv = ds.getInvestition(d[1]);
      ...
      if (d[2].equalsIgnoreCase("Beschreibung")) {
         ...
         if (d.length >= 7) {
            ...
            System.out.println("130802-0033 Daten für Investition fehlerhaft");
         }
         ...
         if (d[3].equalsIgnoreCase("Wohnung")) {
            ...
            if (ds.wohnungsanteilFlaeche == null) {
               ...
               for (int i = 0; i < ds.wohnungen.length; i++) {
                  summe += ds.wohnungen[i].faeche;
               }
               for (int i = 0; i < ds.wohnungen.length; i++) {
                  ds.wohnungsanteilFlaeche[i] =  (ds.wohnungen[i].faeche * 100)                         / summe;
               }
            }
